...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === "calendarSyncAlarm") {
    ...
    chrome.storage.local.get('tasks', (data) => {
      // Corrected: The task ID is a combination of "task_" and the timestamp.
      // The alarm name is "notification_" and the timestamp.
      // We need to find the task that matches the timestamp.
...
chrome.notifications.onButtonClicked.addListener((notificationId, buttonIndex) => {
  if (notificationId.startsWith("notification_") && buttonIndex === 0) {
    ...
    const taskId = notificationId.substring("notification_".length); // Correctly gets the timestamp part
    const fullTaskId = `task_${taskId}`; // Reconstructs the full task ID

    chrome.storage.local.get('tasks', (data) => {
      const task = (data.tasks || []).find(t => t.id === fullTaskId); // Find task using the full ID
      if (task && task.link) {
...
// Other parts of background.js remain the same
